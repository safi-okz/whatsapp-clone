    ...
    <div id="Messages" class="pt-1 z-0 overflow-auto fixed h-[calc(100vh-100px)] w-[420px]">
        <div v-for="chat in chats" :key="chat">
            <div @click="openChat(chat)">
                <MessageRowComponent :chat="chat"></MessageRowComponent>
            </div>
        </div>
    </div>
</template>
...
onMounted(async () => {
    if(userDataForChat.value.length){
        console.log('user data ', userDataForChat.value);
        console.log('user data id ', userDataForChat.value[0].id)
...
const openChat = async (chat) => {
    ...
    try{
        ...
        console.log('chat view and opne error ', error.message);
    }
}
</script>
